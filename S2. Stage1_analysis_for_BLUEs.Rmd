...
knitr::opts_knit$set(root.dir = 'F:/Qiuyue/G2F_Prediction_Competition/Maize_GxE_Competition_Data')
```
...
Get the data.
```{r}
g2f = read.csv("./Training_Data/1_Training_Trait_Data_2014_2021_filtered.csv") 
colnames(g2f)
```
...
write.csv(blues.df, file = "G2F 2021 Stage 1 yield BLUEs no filtering.csv", row.names = F, quote = F)

results.list = lapply(X = result.list, FUN = "[[", ... = 2)
results.df = do.call(rbind, results.list)
write.csv(results.df, file = "G2F 2021 individual environment yield heritability no filtering.csv", row.names = F, quote = F)

F.list = lapply(X = result.list, FUN = "[[", ... = 3)
F.df = do.call(rbind, F.list)
write.csv(F.df, file = "G2F 2021 Stage 1 Ftests no filtering.csv", row.names = F, quote = F)

addFactorFromName = function(x, listOfDF){
  y = listOfDF[[x]]
  y$Env_Trait = x
  return(y)
  }
F.list2 = lapply(names(F.list), FUN = addFactorFromName, F.list)
Ftest.df = do.call(rbind, F.list2)
Ftest.df$Env = substr(Ftest.df$Env_Trait, 1, 9)
Ftest.df$Trait = substr(Ftest.df$Env_Trait, 11, 100)
Ftest.df$Env_Trait = NULL
write.csv(Ftest.df, "G2F_2017_stage1_Ftests.csv", row.names = F, quote = F)

vc.list = lapply(X = result.list, FUN = "[[", ... = 4)
vc.df = do.call(rbind, vc.list)
write.csv(vc.df, file = "G2F 2021 Stage 1 variance components no filtering.csv", row.names = F, quote = F)
```
